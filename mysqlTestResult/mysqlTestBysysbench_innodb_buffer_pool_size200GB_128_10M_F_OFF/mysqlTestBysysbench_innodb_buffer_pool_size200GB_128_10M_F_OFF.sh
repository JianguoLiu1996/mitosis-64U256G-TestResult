...
function startMysql(){
	...
	sleep 5s
	echo "===success start mysql==="

	echo "===disable Auto-numa==="
	sudo echo 0 > /proc/sys/kernel/numa_balancing
	sleep 5s
	echo "===success disable Auto-numa==="
...
function startMysqlWithPageReplication(){
        #sudo /etc/init.d/mysql restart
        echo "===begin to stop mysql==="
        sudo service mysql stop
        sleep 5s
        ...
        echo "===begin to start mysql==="
        sudo numactl -r 0-3 service mysql start
        echo "success start mysql==="
        sleep 30s
}
...
function alltest(){
	...
	NUMBER=1nd
	rwtest
	OUTPUTPATH=../mysqlTestBysysbench_innodb_buffer_pool_size200GB_128_10M_F_OFF_2nd/
	NUMBER=2nd
	rwtest
	OUTPUTPATH=../mysqlTestBysysbench_innodb_buffer_pool_size200GB_128_10M_F_OFF_3nd/
        NUMBER=3nd
	rwtest
